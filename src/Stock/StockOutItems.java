...
public class StockOutItems extends javax.swing.JFrame {
    ...
    public void getDetails(){
        ...
        try {
                ...
                String query ="select * from withdrawn_stock where type='W';";
    ...
    private void initComponents() {
        ...
        jPanel3.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 0, 190, 70));
        ...
        withdrawTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "W ID", "Item Name", "W/Qty", "W Date", "Issued By", "Remarks"
            }
        ) {
        ...
        jPanel3.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 370, 750, 220));
        ...
        jPanel3.add(btn, new org.netbeans.lib.awtextra.AbsoluteConstraints(620, 70, 100, 40));
        ...
        jLabel5.setText("Withdraw Quantity");
        ...
        jButton1.setText("Add Condemned Items");
        jButton1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton1ActionPerformed(evt);
            }
        });
        jPanel3.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(570, 313, 160, 30));
        ...
        jPanel2.add(jPanel3);
        jPanel3.setBounds(100, 10, 820, 680);
    ...
    private void btnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActionPerformed
        int input = JOptionPane.showConfirmDialog(this, "Do you want to add Withdraw Details?");
        if(input == JOptionPane.YES_OPTION){
            try {

                Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/surakim_db","root","");
                Statement st=con.createStatement();
                String query ="select iSID,`using` from inventory_stock where iSName='"+types.getSelectedItem()+"';";
                ResultSet rs=st.executeQuery(query);
                rs.next();
                ...
                if(Integer.valueOf(withdraw.getText())<=Integer.valueOf(availble.getText())){
                    query ="insert into withdrawn_stock(wsISID,wsQTy,wsDate,issued_by,remarks) values("+id+","+withdraw.getText()+",curdate(),'"+issuedby.getText()+"','"+remarks.getText()+"');";
                    int r=st.executeUpdate(query);
                    if(btn.getText().equals("Issue")){
                        int a=Integer.valueOf(using)+Integer.valueOf(withdraw.getText());
                        query ="update inventory_stock set `using`="+a+" where iSID="+id+";";
...
//                    sb.setVisible(true);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
       CondemnedItems d=new CondemnedItems();
       this.dispose();
       d.setVisible(true);
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        try {
            ...
            String query ="select iSName from inventory_stock where iSName like '%"+name.getText()+"%';";
            ResultSet rs=st.executeQuery(query);
            while(rs.next()){
    ...
    private void typesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_typesActionPerformed
        ...
        try {
            ...
            String query ="select iSQTy,item_type from inventory_stock where iSName='"+types.getSelectedItem()+"';";
            ResultSet rs=st.executeQuery(query);
            rs.next();
            if(rs.getString("item_type").equals("Non-consumable Items")){
                btn.setText("Issue");
            }
            else{
                btn.setText("Widthraw");
            }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        if(input == JOptionPane.YES_OPTION){
            try {
                ...
                rs1.next();
                String wid=rs1.getString("wsISID");
                int a=Integer.valueOf(availble.getText())+Integer.valueOf(rp[2]);
                String query1 ="select `using` from inventory_stock where iSID='"+wid+"';";
                ResultSet rs=st2.executeQuery(query1);
                rs.next();
                System.out.println(rs.getString("using"));
                int u=Integer.valueOf(rs.getString("using"));
                u=u-Integer.valueOf(rp[2]);
                if(u<0){
                    a+=u;
                    u=0;
                    
                }
                query ="update inventory_stock set iSQTy="+a+",`using`="+u+" where iSID="+wid+";";
                int r=st.executeUpdate(query);
        ...
        if(input == JOptionPane.YES_OPTION){
            try {
                ...
                if(Integer.valueOf(withdraw.getText())<=(Integer.valueOf(availble.getText())+Integer.valueOf(rp[2]))){
                    ...
                    r=st.executeUpdate(query);
                    if(btn.getText().equals("Issue")){
                        int b=Integer.valueOf(using)+Integer.valueOf(withdraw.getText());
                        query ="update inventory_stock set `using`="+b+" where iSID="+id+";";
                        r=st.executeUpdate(query);
                    }
                    WithdrawBill sb=new WithdrawBill();
                    sb.setVisible(true);
