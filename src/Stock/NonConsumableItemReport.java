...
public class NonConsumableItemReport extends javax.swing.JFrame {


    public NonConsumableItemReport() {
        initComponents();
        getDetails();
    }
    ...
    void getDetails(){
        try {
            ...
            while(rs.next()){
                ...
                rp[3]=rs.getString("using");
                
                
                try{
                    query ="select sum(wsQTy) as damage from withdrawn_stock where wsISID="+rs.getString("iSID")+" and type='D' group by wsISID;";
                    ResultSet rs1=st1.executeQuery(query);
                    rs1.next();
                    rp[4]=rs1.getString("damage");
                    rp[5]=String.valueOf(Integer.valueOf(rp[2])+Integer.valueOf(rp[3])+Integer.valueOf(rp[4]));
                }
                catch(Exception e){
                    rp[4]="0";
                    rp[5]=String.valueOf(Integer.valueOf(rp[2])+Integer.valueOf(rp[3])+Integer.valueOf(rp[4]));
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Non-consumable Item Report");
        jPanel3.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(140, 30, -1, -1));
        ...
        inventoryStockTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Item Name", "Added Date", "Available Qty", "Using Qunatity", "Damage Quantity", "Total"
            }
        ) {
    ...
    private void typesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_typesActionPerformed
        ...
        try {
            ...
            String query ="select * from inventory_stock where iSName='"+types.getSelectedItem()+"' and item_type='Non-consumable Items';";
            ResultSet rs=st.executeQuery(query);
            while(rs.next()){
                rp[0]=rs.getString("iSName");
                rp[1]=rs.getString("iSDate");
                ...
                try{
                    ...
                    rs.next();
                    rp[4]=rs.getString("damage");
                    rp[5]=String.valueOf(Integer.valueOf(rp[2])+Integer.valueOf(rp[3])+Integer.valueOf(rp[4]));
                }
                catch(Exception e){
                    rp[4]="0";
                    rp[5]=String.valueOf(Integer.valueOf(rp[2])+Integer.valueOf(rp[3]));
                }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        for(int i=0;i<inventoryStockTable.getRowCount();i++){
            ...
            String type=(String) inventoryStockTable.getValueAt(i ,0);
            int count=Integer.valueOf((String) inventoryStockTable.getValueAt(i ,5));
            dataset.setValue(count, "Items", type);
